...
@section('page_content')
    <div class="box">
        <div class="box-body pb-20">
            <form class="form-horizontal" action="{{ url(\Config::get('adminPrefix').'/merchants') }}" method="GET">
                    ...
                    <div class="col-md-12 f-14">
                        <div class="d-flex flex-wrap justify-content-between">
                            <div class="d-flex flex-wrap">
                                <!-- Date and time range -->
                                <div class="pr-25">
                                    <label class="fw-bold mb-1">{{ __('Date Range') }}</label><br>
                                ...
                                <!-- User -->
                                <div class="pr-25">
                                    <label class="fw-bold mb-1" for="user">{{ __('User') }}</label><br>
                                    <div class="input-group">
                                        <input id="user_input" type="text" name="user" placeholder="{{ __('Enter Name') }}" class="form-control f-14" value="{{ empty($user) ?  $user : $getName->first_name.' '.$getName->last_name }}"
    ...
    $(document).ready(function()
    {
        ...
        $('#user_input').autocomplete(
        {
            source:function(req,res)
            {
                ...
                {
                    $.ajax({
                        url:'{{ url(\Config::get('adminPrefix').'/merchants/userSearch') }}',
                        dataType:'json',
                        type:'get',
                        data:{
                            search:req.term
